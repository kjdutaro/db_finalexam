...
function getInboxDocuments()
{
    ...
    $inboxQuery = "SELECT recipient.track_id, document.document_id, document.DateCreated, document.file_path,
                    document.file_name, personnel.name, trackdetails.origin_office, document.status, document.isAccomplished
                    FROM Document
...
function getOutboxDocuments()
{
    ...
    $outboxQuery = "SELECT sender.track_id, document.document_id, document.DateCreated, document.file_path,
                    document.file_name, personnel.name, trackdetails.origin_office, document.status, document.isAccomplished 
                    FROM Document
                    JOIN trackdetails ON document.document_id = trackdetails.document_id
                    JOIN recipient ON trackdetails.track_id = recipient.track_id
